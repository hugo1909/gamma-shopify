  ...
  return (
    <div className="grid grid-cols-[80px_1fr_20px] items-start gap-4 border-t border-default py-4 first:border-t-0 md:py-6">
      ...
      <div className="flex max-w-[172px] flex-col self-start md:max-w-[212px] md:self-center">
        <a href={item.url} className="text-sm text-grey-900 font-bold" title={item.title}>
          {productTitle}
        </a>
        <CartInfo item={item} infos={infos} properties={properties} isGiftCard={item.gift_card || item.product_type === 'Gift card'} />
