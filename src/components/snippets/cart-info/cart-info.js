...
const IGNORE_PROPERTIES = ['timestamp', '__shopify_send_gift_card_to_recipient']

function CartInfo({ infos, properties, item }) {
  ...
  return (
    <div>
      {typeof item.variant_title !== 'undefined' ? <p className="pt-[3px] text-sm font-normal -tracking-wider text-grey-700 capitalize">{item.variant_title.toLowerCase()}</p> : ''}
      {infos.map((info) => (
