...
class Carousel extends HTMLElement {
  constructor() {
    ...
    this.caroselMain = select(selectors.CAROUSEL_MAIN, this)
    this.dots = null
    this.currentIndex = 0
    ...
    this.interval = null
    this.pause = 6000
    this.hasAutoplay = (this.dataset.hasAutoplay === 'true')
    this.hasInfiniteScroll = (this.dataset.hasInfiniteScroll === 'true')
    this.drag = {
      press: false,
      startX: 0,
      scrollLeft: 0,
    ...
    this.viewSpecs = select(selectors.VIEW_SPECS, this) || null
    this.init()
    this.initNavigation()
  ...
  init() {
    if (this.track && this.track.children.length > 0) {
      ...
      this.gapSlide = this.getViewport(this.track)
  ...
  initNavigation() {
    ...
    if (this.viewSpecs != null) {
      this.viewSpecs.addEventListener('click', this.onviewSpecs.bind(this))
    }
  }
  ...
  onviewSpecs(e) {
    ...
    if (target) {
      const index = parseInt(target.getAttribute('data-view-specs'))
      this.setIndex(index)
    }
  }
  ...
  updateNavigation(index) {
    ...
    const carouselTrack = this.querySelector(selectors.TRACK_SELECTOR)
    if (currentPath.indexOf('/blogs/') !== -1) {
      carouselTrack.classList.add('max-width-none')
  ...
  previous() {
    if (this.hasInfiniteScroll) {
      ...
      this.setIndex(index)
      const listItems = this.querySelector(selectors.TRACK_SELECTOR)
      const lastItem = listItems.lastChild
      listItems.insertBefore(lastItem, listItems.childNodes[1])
    }

    if (!this.hasInfiniteScroll) {
      let index = this.currentIndex - 1
      if (this.currentIndex === 0) index = 0
      if (!this.hasInfiniteScroll && this.currentIndex === 0) {
        index = this.maxIndex - 1
      }
      this.setIndex(index)
    }
  ...
  next() {
    if (this.hasInfiniteScroll) {
      ...
      this.setIndex(index)
      const listItems = this.querySelector(selectors.TRACK_SELECTOR)
      const firstItem = listItems.childNodes[1]
    ...
    if (!this.hasInfiniteScroll) {
      ...
      if (!this.hasInfiniteScroll && this.currentIndex === this.maxIndex - 1) {
        index = 0
      }
      this.setIndex(index)
    }
  ...
  initCurrent() {
    const currentEls = document.querySelectorAll('.product-gallery-item.current')
    const currentEl = document.querySelector(`carousel-main div[data-index="${this.currentIndex}"]`)

    if (currentEls.length) {
      currentEls.forEach((item) => {
